...
sendRequest(callOptions, callback) {
  ...
  let finalCallOption = { ...this.options }
  let uri;
  if (finalCallOption.query)
    uri = this.constructUri(finalCallOption.serviceNowTable, finalCallOption.query);
  else
    uri = this.constructUri(finalCallOption.serviceNowTable);
  /**
  ...
  const requestOptions = {
      method: finalCallOption.method,
      auth: {
      user: this.options.username,
      pass: this.options.password,
    },
    baseUrl: this.options.url,
     uri: `/api/now/table/${finalCallOption.serviceNowTable}?sysparm_limit=1`
  };
